...
function sirsiAvailability (result, search_by, info) {
  try {
    ...
    var availabilityData = {
      'available': parseInt(availability['totalAvailable'].toString(), 10),
      'total': parseInt(availability['copies'][0].match(/(\d+)$/)[1], 10),
      'wait': parseInt(availability['holdCounts'][0].match(/(\d+)$/)[1], 10)
    };
...
function overdriveAvailability (result, type) {
  try {
    ...
    var availabilityData = {
      'available': parseInt(availability.attr('data-copiesavail'), 10),
      'total': parseInt(availability.attr('data-copiestotal'), 10),
